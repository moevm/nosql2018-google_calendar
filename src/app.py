...
def main():
    ...
    return render_template('main.html')
            ...
            # username = str(os.path.abspath(filename))
            # parse_to_mongo_user(username)
...
def screen_5(is_year):
    cursor = mongo.db.users.find({"email": "kochnevaolga74@gmail.com"})
    ...
    for id in cursor:
        user_id = id.get('_id')
        count = mongo.db.events.find({"user_id": user_id, "organizer": "kochnevaolga74@gmail.com", "start": {'$gte': cur_date}}).count()
...
def parse_to_mongo_events(user_id, user):  # закидываем его мероприятия
    ...
    for event in json_file:
        dtstart = dateutil.parser.parse(event["DTSTART"]) + timedelta(hours=3) # преобразуем в человеческий формат дату плюс 3 часа потому
